...
exports.create = (req, res, next) => {
  ...
  Product.findOne({ where: { id: productId }, raw: true }).then((product) => {
    if (!product) {
    ...
    console.log(product);
    if (
      (product.category === "Multimedia" && discount <= 20) ||
      (product.category !== "Multimedia" && discount <= 50)
    ) {
      console.log("OK");
      ...
      promotion
        ...
        .then((result) => {
          logController.saveLog(
            ...
            result.id,
            "Discount Added",
            `A Discount on the following the product has been added:
            Product ID: ${result.productId}, 
            Discount Percentage: ${result.discount},
            Stock (When discount added): ${result.currentStock},
            Location ID: ${result.centerId}`
          );
          res.status(201).json({
      ...
      if (product.category === "Multimedia") {
        message =
          "Product discount of Multimedia category must not exceed 20%.";
      } else {
...
exports.getMyPromotions = (req, res, next) => {
  ...
  if (hour >= 8 && hour <= 12) {
    ...
    Promotion.findAll({
      include: [{ model: Product, where: { category: req.category } }],
      where: { centerId: req.centerId, status: "Untreated", day: fullDate },
      raw: true,
    }).then((promotions) => {
...
exports.updatePromotion = (req, res, next) => {
  ...
  Promotion.update(
    { status: status, comment: comment, currentStock: currentStock },
    ...
    .then((result) => {
      logController.saveLog(
        ...
        req.id,
        "Processing Discount",
        `Manager of Aisle (${req.category}) processed its Discounts`
      );
      console.log(result);
